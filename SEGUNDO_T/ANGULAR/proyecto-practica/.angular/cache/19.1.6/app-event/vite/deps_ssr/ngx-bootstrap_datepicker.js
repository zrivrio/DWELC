...
var MiniState = class extends import_rxjs2.BehaviorSubject {
  constructor(_initialState, actionsDispatcher$, reducer) {
    super(_initialState);
    const actionInQueue$ = actionsDispatcher$.pipe((0, import_operators.observeOn)(import_rxjs2.queueScheduler));
    const state$ = actionInQueue$.pipe((0, import_operators.scan)((state2, action) => {
      if (!action) {
        return state2;
      }
...
var MiniStore = class _MiniStore extends import_rxjs2.Observable {
  ...
  select(pathOrMapFn) {
    const mapped$ = this.source?.pipe((0, import_operators.map)(pathOrMapFn)) || new import_rxjs2.Observable().pipe((0, import_operators.map)(pathOrMapFn));
...
var TimepickerStore = class _TimepickerStore extends MiniStore {
  constructor() {
    const _dispatcher = new import_rxjs3.BehaviorSubject({
...
var TooltipDirective = class _TooltipDirective {
  ...
  show() {
    ...
    if (this.delay) {
      ...
      this._delaySubscription = (0, import_rxjs4.timer)(this.delay).subscribe(() => {
        showTooltip();
...
var BsDatepickerAbstractComponent = class {
  constructor() {
    ...
    this.customRanges = [];
    this.chosenRange = [];
    this._daysCalendarSub = new import_rxjs5.Subscription();
    this.selectedTimeSub = new import_rxjs5.Subscription();
  }
  set minDate(value) {
...
var BsLocaleService = class _BsLocaleService {
  constructor() {
    ...
    this._locale = new import_rxjs5.BehaviorSubject(this._defaultLocale);
...
var BsDatepickerEffects = class _BsDatepickerEffects {
  ...
  setBindings(container) {
    ...
    container.selectedTime = this._store.select((state2) => state2.selectedTime).pipe((0, import_operators2.filter)((times) => !!times));
    container.daysCalendar$ = this._store.select((state2) => state2.flaggedMonths).pipe((0, import_operators2.filter)((months2) => !!months2));
    container.monthsCalendar = this._store.select((state2) => state2.flaggedMonthsCalendar).pipe((0, import_operators2.filter)((months2) => !!months2));
    container.yearsCalendar = this._store.select((state2) => state2.yearsCalendarFlagged).pipe((0, import_operators2.filter)((years) => !!years));
    container.viewMode = this._store.select((state2) => state2.view?.mode);
    container.options$ = (0, import_rxjs5.combineLatest)([this._store.select((state2) => state2.showWeekNumbers), this._store.select((state2) => state2.displayMonths)]).pipe((0, import_operators2.map)((latest) => ({
  ...
  registerDatepickerSideEffects() {
    ...
    this._subs.push(this._store.select((state2) => state2.monthsModel).pipe((0, import_operators2.filter)((monthModel) => !!monthModel)).subscribe(() => this._store?.dispatch(this._actions.format())));
    this._subs.push(this._store.select((state2) => state2.formattedMonths).pipe((0, import_operators2.filter)((month) => !!month)).subscribe(() => this._store?.dispatch(this._actions.flag())));
    this._subs.push(this._store.select((state2) => state2.selectedDate).pipe((0, import_operators2.filter)((selectedDate) => !!selectedDate)).subscribe(() => this._store?.dispatch(this._actions.flag())));
    this._subs.push(this._store.select((state2) => state2.selectedRange).pipe((0, import_operators2.filter)((selectedRange) => !!selectedRange)).subscribe(() => this._store?.dispatch(this._actions.flag())));
    this._subs.push(this._store.select((state2) => state2.monthsCalendar).subscribe(() => this._store?.dispatch(this._actions.flag())));
    this._subs.push(this._store.select((state2) => state2.yearsCalendarModel).pipe((0, import_operators2.filter)((state2) => !!state2)).subscribe(() => this._store?.dispatch(this._actions.flag())));
    this._subs.push(this._store.select((state2) => state2.hoveredDate).pipe((0, import_operators2.filter)((hoveredDate) => !!hoveredDate)).subscribe(() => this._store?.dispatch(this._actions.flag())));
    this._subs.push(this._store.select((state2) => state2.dateCustomClasses).pipe((0, import_operators2.filter)((dateCustomClasses) => !!dateCustomClasses)).subscribe(() => this._store?.dispatch(this._actions.flag())));
    this._subs.push(this._store.select((state2) => state2.dateTooltipTexts).pipe((0, import_operators2.filter)((dateTooltipTexts) => !!dateTooltipTexts)).subscribe(() => this._store?.dispatch(this._actions.flag())));
...
var BsDatepickerStore = class _BsDatepickerStore extends MiniStore {
  constructor() {
    const _dispatcher = new import_rxjs5.BehaviorSubject({
...
var BsDatepickerContainerComponent = class _BsDatepickerContainerComponent extends BsDatepickerAbstractComponent {
  ...
  ngOnInit() {
    ...
    this._positionService.event$?.pipe((0, import_operators2.take)(1)).subscribe(() => {
...
var BsDatepickerDirective = class _BsDatepickerDirective {
  ...
  constructor(_config, _elementRef, _renderer, _viewContainerRef, cis) {
    ...
    this.isDestroy$ = new import_rxjs5.Subject();
    this.isDisabled = false;
    this.bsValueChange = new EventEmitter();
    this._subs = [];
    this._dateInputFormat$ = new import_rxjs5.Subject();
    Object.assign(this, this._config);
    this._datepicker = cis.createLoader(_elementRef, _viewContainerRef, _renderer);
    this.onShown = this._datepicker.onShown;
    this.onHidden = this._datepicker.onHidden;
    this.isOpen$ = new import_rxjs5.BehaviorSubject(this.isOpen);
  }
  /**
  ...
  ngAfterViewInit() {
    this.isOpen$.pipe((0, import_operators2.filter)((isOpen) => isOpen !== this.isOpen), (0, import_operators2.takeUntil)(this.isDestroy$)).subscribe(() => this.toggle());
  }
  /**
...
var BsDaterangepickerInlineDirective = class _BsDaterangepickerInlineDirective {
  ...
  initSubscribes() {
    ...
    if (this._datepickerRef) {
      this._subs.push(this._datepickerRef.instance.valueChange.pipe((0, import_operators2.filter)((range) => range && range[0] && !!range[1])).subscribe((value) => {
        this.bsValue = value;
      }));
    }
  }
...
var BsDatepickerInputDirective = class _BsDatepickerInputDirective {
  constructor(_picker, _localeService, _renderer, _elRef, changeDetection) {
    ...
    this._subs = new import_rxjs5.Subscription();
  }
  onChange(event) {
  ...
  ngOnInit() {
    ...
    this._subs.add(this._picker.dateInputFormat$.pipe((0, import_operators2.distinctUntilChanged)()).subscribe(() => {
      this._setInputValue(this._value);
    }));
  }
  ngOnDestroy() {
...
var BsDaterangepickerDirective = class _BsDaterangepickerDirective {
  ...
  constructor(_config, _elementRef, _renderer, _viewContainerRef, cis) {
    ...
    this.bsValueChange = new EventEmitter();
    this._subs = [];
    this._rangeInputFormat$ = new import_rxjs5.Subject();
    this._datepicker = cis.createLoader(_elementRef, _viewContainerRef, _renderer);
...
var BsDaterangepickerInputDirective = class _BsDaterangepickerInputDirective {
  ...
  ngOnInit() {
    ...
    this._subs.add(
      ...
      this._picker.rangeInputFormat$.pipe((0, import_operators2.distinctUntilChanged)()).subscribe(() => {
        this._setInputValue(this._value);
      })
    );
  }
